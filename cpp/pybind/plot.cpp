...
PyObject* c_plot_boxplot(PyObject* args, PyObject* kwargs)
{
	...
	try
	{
		...
		auto NumData = std::make_unique<core::CRealColData>(Data);
		auto series = std::make_unique<CBoxWhiskerSeries>(BW_Chrt, std::move(NumData));
...
PyObject* c_plot_histogram(PyObject* args, PyObject* kwargs)
{
	...
	try
	{
		...
		IF_PYERRRUNTIME_RET(Histogram->GetSeriesSize() == 1, "There is already one series");

		auto NumData = std::make_unique<core::CRealColData>(Data);
		IF_PYERRRUNTIME_RET(NumData == nullptr, "The data is not valid to plot histogram.");

		auto series = std::make_unique<CHistogramSeries>(Histogram, std::move(NumData));
...
void c_plot_show(bool maximize, bool sharedLoop)
{
